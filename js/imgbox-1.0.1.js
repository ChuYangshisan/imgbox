...
// Pass this if window is not defined yet
    ...
    var ImgBox = window.ImgBox = {
        ...
        //存放放大图片的区域容器
        $zoomArea: $('<div class="imgbox-area" style="display: none;"></div>'),     //TODO 增加关闭按钮
        //遮罩层
        $cover: $('<div class="imgbox-cover" style="display: none;"></div>'),
    ...
    /**由区域中心向四周放大, 或向中心收缩*/
    function _radiusScroll(box, size) {  //size:[oldWidth,oldHeight,newWight,newHeight]
        var left = box.position().left - (size[2] - size[0]) / 2;       //position().left 获取的是数值
        var top = box.position().top - (size[3] - size[1]) / 2;
        $(box).css({
            left: left,
            top: top
        });
    ...
    Origin.prototype.configLargeBox = function () {
        var _this = this;
        ...
        //居中显示
        //大图居中
        _centerize(_this.$largeBox, _this.largeImage);
    ...
    * 若大图url==小图url -> 不重新加载, 接着用已有的*/
    ...
    var _scrollFunc = function (e) {
        ...
        //居中显示
        //大图居中
        _radiusScroll(ImgBox.activeOrigin.$largeBox, [w, h, ImgBox.activeOrigin.largeImage.width, ImgBox.activeOrigin.largeImage.height]);
